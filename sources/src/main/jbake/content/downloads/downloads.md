...
__<a href="https://www.apache.org/dyn/closer.cgi/roller/roller-6.1/v6.1.6">Roller 6.1.6</a> (&quot;best available&quot;)__
...
Note: When downloading from a mirror, be sure to verify that checksums and/or signatures are correct. To do so, use the checksum and signature files from the main Apache distribution site at <a class="external" href="https://downloads.apache.org/roller/">downloads.apache.org/roller</a>.

The PGP signatures can be verified using PGP or GPG. First download the <a href="https://downloads.apache.org/roller/KEYS">KEYS</a> as well as the ``asc`` signature file for the particular distribution. Make sure you get these files from the <a href="https://downloads.apache.org/roller/">main distribution directory,</a> rather than from a mirror. Then verify the signatures using ``pgpk`` as follows:
...
Alternatively you can also verify the SHA-256 or SHA-512 checksum for each file (Roller 6.1.6 ships both, as ``.sha256`` and ``.sha512`` files). Apache Open Office provides a nice guide to verifying checksum here <a href="https://www.openoffice.org/download/checksums.html">How to verify the integrity of the downloaded file?</a>
